...
{
    public static class laasistentes_cancel
    {
        [FunctionName("laasistentes_cancel")]
            ...
            ILogger log)
        {
            ...
            if ((string)req.Method == "POST")
            {
                ...
                {
                    List<Sesion> sesion = tableSesiones.CreateQuery<Sesion>().AsQueryable<Sesion>().Where(e => e.PartitionKey == "Sesiones" && e.fecha_evento>=fecha_evento_inicio_date && e.fecha_evento <= fecha_evento_fin_date).ToList();
                ...
                * ***********************************************************/
                    ...
                    else
                    {
                        ...
                        List<Asistente> asistente = tableAsistentes.CreateQuery<Asistente>().AsQueryable<Asistente>().Where(e => e.PartitionKey == "0" && e.idSesion == sesion_orden[0].idSesion && e.nomina_asistente == usuario[0].nomina).ToList();
